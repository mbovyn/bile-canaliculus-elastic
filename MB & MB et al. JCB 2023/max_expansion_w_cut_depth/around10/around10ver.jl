...
# vs=unique(dfse[i],:v)[:,:v]
# ebv=groupby(sort(unique(dfse[i]),:x),[:v,:ablated])
# plot([scatter3d(ebv[(vs[j],0)],x=:x,y=:y,z=:z,name="before"),
#     scatter3d(ebv[(vs[j],1)],x=:x,y=:y,z=:z,name="after"),
#     scatter3d(x=ebv[(vs[j],0)][1,:nickc].+[-1,0,1]*ebv[(vs[j],0)][1,:nickw]/2,
...
#         title=string("Bulkhead young's = ",dfse[i][1,:bulkheadyoungs],", v = ",vs[j])
#     #height=400,
#     #width=500
# ))
...
for subgg in gg
    addtraces!(pc,scatter(subgg,x=:v,y=subgg.maxx/subgg.maxz[1],name=string(subgg.bulkheadyoungs[1])))
end
pc
...
plot(scatter(y=vcrit[1][2:7],x=c[1][2:7]))
